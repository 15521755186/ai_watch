...
uint64_t SystemInfo::boot_start_us_ = 0;

void SystemInfo::RecordBootTime() {
    boot_start_us_ = esp_timer_get_time();
}

uint64_t SystemInfo::GetBootElapsedMs() {
    if (boot_start_us_ == 0) return 0;
    return (esp_timer_get_time() - boot_start_us_) / 1000;
}
