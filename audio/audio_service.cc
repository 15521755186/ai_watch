...
void AudioService::Initialize(AudioCodec* codec) {
    ...
    opus_encoder_->SetComplexity(4);  // 从 0 提升到 4，语音识别准确率更高，CPU 开销几乎不变
...
void AudioService::AudioOutputTask() {
    while (true) {
        ...
        if (stop_audio_immediately_) {
            if (codec_->output_enabled()) {
                codec_->EnableOutput(false);
            }
            stop_audio_immediately_ = false;
            continue;
        }
...
bool AudioService::PushPacketToDecodeQueue(std::unique_ptr<AudioStreamPacket> packet, bool wait) {
    ...
    if (audio_decode_queue_.size() >= MAX_DECODE_PACKETS_IN_QUEUE) {
        if (wait) {
            audio_queue_cv_.wait(lock, [this]() {
                return stop_audio_immediately_ || audio_decode_queue_.size() < MAX_DECODE_PACKETS_IN_QUEUE;
            });
            if (stop_audio_immediately_) {
                return false;
            }
...
void AudioService::PlaySound(const std::string_view& sound) {
    uint32_t sequence = ++playback_sequence_;
    stop_audio_immediately_ = false;
    const char* data = sound.data();
    size_t size = sound.size();
    for (const char* p = data; p < data + size; ) {
        if (stop_audio_immediately_ || sequence != playback_sequence_) {
            break;
        }
...
void AudioService::StopAudioImmediately() {
    ++playback_sequence_;
    stop_audio_immediately_ = true;
    std::lock_guard<std::mutex> lock(audio_queue_mutex_);
    audio_playback_queue_.clear();
    ...
    audio_testing_queue_.clear();
    audio_queue_cv_.notify_all();
    // Note: Don't reset the flag here, let AudioOutputTask handle it
}
